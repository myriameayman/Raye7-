...
def create
    unless(params[:id==nil])
          ...
          unless(params[:id] == nil)
            ...
            @user = current_user
            @points = getpoints(@request.distance)
            @owner.budget = @owner.budget + @points
            @user.budget = @user.budget - @points
            if(@request.seats>0 and @user.budget>=@points)
...
# method getpoints to calculate points (budget) according to the distance estimated for the trip
# added by: Ahmed Amr Ismail
def getpoints(distance)
    liters = distance/10
